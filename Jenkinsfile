pipeline {
    ...
    environment {
        ...
        FRONTEND_IMAGE = '438465127898.dkr.ecr.us-east-1.amazonaws.com/angular-frontend'
        FRONTEND_TAG = 'latest'
        BACKEND_IMAGE = '438465127898.dkr.ecr.us-east-1.amazonaws.com/angular-backend'
        BACKEND_TAG = 'latest'
            ...
            steps {
                script {
                    withCredentials([string(credentialsId: 'aws-ecr-token', variable: 'ECR_PASSWORD')]) {
                        sh 'echo "$ECR_PASSWORD" | docker login --username AWS --password-stdin 438465127898.dkr.ecr.us-east-1.amazonaws.com'
                    }
                }
            }
        }
